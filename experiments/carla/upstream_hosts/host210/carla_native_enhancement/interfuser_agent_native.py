"""
InterfuserAgent with CARLA Native Sensor Enhancement

This agent supports CARLA-level sensor upgrades (high FPS, high resolution, no noise)
without post-processing or model retraining.

Usage:
1. Set NATIVE_ENHANCE environment variable (e.g., "high_fps,high_res,no_noise")
2. Deploy this file to leaderboard/team_code/interfuser_agent.py
3. Run evaluation with run_evaluation_native.sh

Key modifications:
- sensors() method returns configuration based on NATIVE_ENHANCE
- No post-processing applied (native sensor quality)
- Agent automatically adapts to world tick rate (20Hz or 40Hz)
"""
...
# 🔥 新增：导入 CARLA 原生增强配置 🔥
# When deployed to leaderboard/team_code/, we need to import from project root
import sys
import os as _os_for_import
_project_root = _os_for_import.path.abspath(_os_for_import.path.join(_os_for_import.path.dirname(__file__), '../..'))
if _project_root not in sys.path:
    sys.path.insert(0, _project_root)

from carla_native_enhancement.native_config_parser import load_config_from_env
...
class InterfuserAgent(autonomous_agent.AutonomousAgent):
    def setup(self, path_to_conf_file):
        ...
        self.world_settings_applied = False  # 用于确保只应用一次世界设置
        
        print("\n" + "=" * 70)
        print("🔧 CARLA Native Sensor Enhancement Initialized")
        print("=" * 70)
        config_summary = {
            "native_enhance": ",".join(self.native_config.tokens),
            "high_fps": self.native_config.high_fps,
            "high_res": self.native_config.high_res,
            "no_noise": self.native_config.no_noise,
            "gaussian_noise_sigma": self.native_config.gaussian_noise_sigma,
            "frame_rate": f"{self.native_config.frame_rate}Hz",
            "resolution": f"{self.native_config.camera_width}x{self.native_config.camera_height}",
            "motion_blur": self.native_config.motion_blur_intensity,
            "lens_flare": self.native_config.lens_flare_intensity,
        }
    ...
    def sensors(self):
        """
        Define sensors with CARLA native enhancements.
        
        Configuration is loaded from NATIVE_ENHANCE environment variable.
        """
        ...
        return [
            ...
            },
            {
                ...
                "pitch": 0.0,
                "yaw": -60.0,
                "width": cfg.camera_width // 2,  # Side cameras use half resolution
                "height": cfg.camera_height // 2,
                "fov": 100,
                "id": "rgb_left",
                # Native enhancements
                "sensor_tick": cfg.sensor_tick,
            ...
            },
            {
                ...
                "pitch": 0.0,
                "yaw": 0.0,
                "sensor_tick": cfg.sensor_tick,  # Match world tick rate
                "id": "imu",
            },
            {
                ...
                "pitch": 0.0,
                "yaw": 0.0,
                "sensor_tick": 0.01,  # Keep GNSS at 100Hz
                "id": "gps",
            },
            {
                "type": "sensor.speedometer",
                "reading_frequency": cfg.frame_rate,  # Match world tick rate
                "id": "speed",
            },
        ]
    ...
    def tick(self, input_data):
        """
        Process sensor data (no post-processing, native quality only).
        """
        # Extract RGB images (no processing)
        rgb = cv2.cvtColor(input_data["rgb"][1][:, :, :3], cv2.COLOR_BGR2RGB)
        rgb_left = cv2.cvtColor(input_data["rgb_left"][1][:, :, :3], cv2.COLOR_BGR2RGB)
        rgb_right = cv2.cvtColor(input_data["rgb_right"][1][:, :, :3], cv2.COLOR_BGR2RGB)

        # Standalone gaussian noise injection (cannot be combined with other tokens)
        ...
        if sigma and sigma > 0:
            ...
            rgb = _add_gaussian_noise_uint8(rgb, sigma)
    ...
    def run_step(self, input_data, timestamp):
        ...
        # 🔥 应用世界设置（同步模式 + 固定时间步长）🔥
        # 这可以防止 CARLA 超时和传感器流问题
        if not self.world_settings_applied:
            try:
                # 获取世界对象（通过传感器数据）
                # 注意：在 leaderboard 中，world 对象通过 scenario 传递
                # 我们需要从 input_data 中获取
                client = carla.Client('localhost', int(os.environ.get('PORT', '2000')))
                client.set_timeout(30.0)
                world = client.get_world()
                
                settings = world.get_settings()
                current_sync = bool(getattr(settings, 'synchronous_mode', False))
                current_delta = getattr(settings, 'fixed_delta_seconds', None)
                expected_delta = 1.0 / float(self.native_config.frame_rate)
                print(f"World settings observed: sync_mode={current_sync}, delta={current_delta}")
                if (current_sync is not True) or (current_delta is None) or (abs(float(current_delta) - expected_delta) > 1e-6):
                    print(f"⚠ Warning: World settings mismatch. Expected sync_mode=True, delta={expected_delta:.4f}s ({self.native_config.frame_rate}Hz)")
                self.world_settings_applied = True
            except Exception as e:
                print(f"⚠ Warning: Could not apply world settings: {e}")
                print("  Continuing without synchronous mode...")
                self.world_settings_applied = True  # 避免重复尝试
