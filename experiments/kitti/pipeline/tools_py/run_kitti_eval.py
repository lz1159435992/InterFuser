...
SUPPORT_ROOT = os.path.join(PROJECT_ROOT, "experiments", "kitti", "support_files")
DEFAULT_KITTI_ROOT = os.path.join(PROJECT_ROOT, "data", "kitti")
DEFAULT_OUT_ROOT = os.path.join(PROJECT_ROOT, "results", "raw", "rq1", "kitti_eval")
DEFAULT_KITTI_OBJECT_ROOT = os.path.join(DEFAULT_KITTI_ROOT, "object")
...
def _find_pkl_to_dir_script() -> str:
    candidates = [
        os.path.join(TOOLS_DIR, "v100", "tools", "py", "pkl_to_dir.py"),
    ]
    for p in candidates:
        if os.path.isfile(p):
            return p
    raise FileNotFoundError(
        "pkl_to_dir.py not found in expected locations. "
        "Place it under experiments/kitti/pipeline/tools_py/v100/tools/py/."
    )
...
def eval_virconv(
    *,
    ...
    pkl_to_dir = _find_pkl_to_dir_script()
...
def main() -> None:
    ...
    ap.add_argument(
        "--virconv-cfg-file",
        default=os.path.join(SUPPORT_ROOT, "virconv_models", "VirConv-L", "default", "VirConv-L.yaml"),
    )
    ap.add_argument(
        "--virconv-ckpt",
        default=os.path.join(SUPPORT_ROOT, "virconv_models", "VirConv-L", "default", "ckpt", "checkpoint_epoch_50.pth"),
    )
    ...
    ap.add_argument("--did-m3d-root", default=os.path.join(SUPPORT_ROOT, "did_m3d"))
    ...
    ap.add_argument(
        ...
        default=os.path.join(PROJECT_ROOT, "results", "cache", "didm3d_depth"),
        help="Global cache root for resized DID-M3D depth_dense, bucketed by target size.",
    )

    ap.add_argument("--kitti-native-eval-bin", default=os.path.join(SUPPORT_ROOT, "kitti_native_evaluation", "evaluate_object_3d_offline"))
