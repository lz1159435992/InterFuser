...
class AutumnModel(object):
    ...
    def predict(self, img):
        ...
        cnn_output = self.fc3.eval(feed_dict={self.x: [image], self.keep_prob: 1.0}, session=self.sess)
        self.steps.append(cnn_output)
        if len(self.steps) > 100:
            self.steps.pop(0)
        output = self.y.eval(feed_dict={self.x: [image], self.keep_prob: 1.0}, session=self.sess)
        angle = output[0][0]
        return angle
        ...
        with open(args.input_file) as f:
            with open(output_file, 'w') as csvfile:
                ...
                for row in reader:
                    ...
                    writer.writerow({'frame_id': row['frame_id'], 'steering_angle': result})
