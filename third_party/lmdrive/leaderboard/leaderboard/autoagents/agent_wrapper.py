        ...
        for sensor_spec in self._agent.sensors():
            ...
            # These are the sensors spawned on the carla world
                ...
                if sensor_spec['type'].startswith('sensor.camera.semantic_segmentation'):
                    ...
                    if 'motion_blur_intensity' in sensor_spec:
                        try:
                            bp.set_attribute('motion_blur_intensity', str(sensor_spec['motion_blur_intensity']))
                        except Exception:
                            pass
                    if 'motion_blur_max_distortion' in sensor_spec:
                        try:
                            bp.set_attribute('motion_blur_max_distortion', str(sensor_spec['motion_blur_max_distortion']))
                        except Exception:
                            pass
                    if 'bloom_intensity' in sensor_spec:
                        try:
                            bp.set_attribute('bloom_intensity', str(sensor_spec['bloom_intensity']))
                        except Exception:
                            pass
                    if 'lens_flare_intensity' in sensor_spec:
                        try:
                            bp.set_attribute('lens_flare_intensity', str(sensor_spec['lens_flare_intensity']))
                        except Exception:
                            pass
                    if 'lens_circle_falloff' in sensor_spec:
                        try:
                            bp.set_attribute('lens_circle_falloff', str(sensor_spec['lens_circle_falloff']))
                        except Exception:
                            pass
                    if 'lens_k' in sensor_spec:
                        try:
                            bp.set_attribute('lens_k', str(sensor_spec['lens_k']))
                        except Exception:
                            pass
                    if 'lens_kcube' in sensor_spec:
                        try:
                            bp.set_attribute('lens_kcube', str(sensor_spec['lens_kcube']))
                        except Exception:
                            pass
                    if 'lens_circle_multiplier' not in sensor_spec:
                        bp.set_attribute('lens_circle_multiplier', str(3.0))
                    if 'lens_circle_falloff' not in sensor_spec:
                        bp.set_attribute('lens_circle_falloff', str(3.0))
                    if 'chromatic_aberration_intensity' not in sensor_spec:
                        bp.set_attribute('chromatic_aberration_intensity', str(0.5))
                    if 'chromatic_aberration_offset' not in sensor_spec:
