#!/bin/bash
#
# 安全运行剩余 7 �?CARLA 原生增强配置（分阶段，避免显存溢出）
...
echo "将运行以下任�?/ Will run the following jobs:"
NUM_JOBS=${#jobs[@]}
for i in "${!jobs[@]}"; do
    item="${jobs[$i]}"
    EVAL_TYPE_ITEM="${item%%:*}"
...
for i in "${!jobs[@]}"; do
    ...
    echo "需要空闲显�?/ Required free mem (MB): ${REQ_MB}"
    if [ "${SKIP_JOB}" = "1" ]; then
    ...
    while true; do
        ...
        echo "  �?当前�?GPU 满足需�?(req_free_mem_mb=${REQ_MB})�?{WAIT_GPU_MEM_SEC}s 后重�?.."
    ...
    echo "  �?进程已启�?/ Process started, PID: ${PID} (reserve_mb=${RESERVE_MB})"
    echo "  等待 ${LAUNCH_GAP_SEC} 秒后启动下一�?/ Waiting ${LAUNCH_GAP_SEC}s before next..."
    echo ""
