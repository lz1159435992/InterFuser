...
class SensorInterface(object):
    def __init__(self):
        ...
        queue_timeout_env = os.environ.get('SENSOR_QUEUE_TIMEOUT', '')
        try:
            self._queue_timeout = float(queue_timeout_env) if queue_timeout_env else 10 # default: 10
        except ValueError:
    ...
    def get_data(self):
        try: 
            ...
            received = set(data_dict.keys()) if 'data_dict' in locals() else set()
            missing = sorted(list(expected - received))
            received_list = sorted(list(received))

            speed_fallback = os.environ.get('SPEED_FALLBACK_ON_TIMEOUT', '1').strip().lower() in {
                '1', 'true', 'yes', 'y', 'on'
            }
            if speed_fallback and missing == ['speed']:
                data_dict['speed'] = (GameTime.get_frame(), {'speed': 0.0})
                return data_dict

            raise SensorReceivedNoData(
                "A sensor took too long to send their data; timeout={:.1f}s; received={}; missing={}".format(
                    self._queue_timeout, received_list, missing
                )
            )
