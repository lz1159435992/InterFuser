...
TEAM_CODE_DIR="/path/to/project/leaderboard/team_code"
BACKUP_DIR=$1
...
if [ -z "$BACKUP_DIR" ]; then
    ...
    if [ -n "$LATEST_BACKUP" ]; then
        echo "  最近备�? $LATEST_BACKUP"
        read -p "是否使用此备份恢�? (Y/n) " -n 1 -r
        echo
        if [[ ! $REPLY =~ ^[Nn]$ ]]; then
...
if [ ! -d "$BACKUP_DIR" ]; then
    echo "错误: 备份目录不存�? $BACKUP_DIR"
    exit 1
fi

echo "从备份恢复文�? $BACKUP_DIR"
echo ""
...
if [ -f "${BACKUP_DIR}/interfuser_agent.py.bak" ]; then
    ...
    echo "  �?删除 interfuser_agent.py (原本不存�?"
...
if [ -f "${BACKUP_DIR}/data_processor.py.bak" ]; then
    ...
    echo "  �?删除 data_processor.py (原本不存�?"
...
if [ -f "${BACKUP_DIR}/data_processor_config.py.bak" ]; then
    ...
    echo "  �?删除 data_processor_config.py (原本不存�?"
