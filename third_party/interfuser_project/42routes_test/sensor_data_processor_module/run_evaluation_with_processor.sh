...
# 设置结果保存路径（带时间戳和配置标识�?TIMESTAMP=$(date +%Y%m%d_%H%M%S)
...
if [ $? -eq 0 ]; then
    ...
    echo "  cd /path/to/project/evaluation_scripts"
...
echo ""
echo "📖 查看结果:"
echo "  bash /path/to/project/evaluation_scripts/view_results.sh ${CHECKPOINT_ENDPOINT}"
echo ""
