...
export CARLA_ROOT=/path/to/project/carla
...
case $EVAL_TYPE in
    ...
    *)
        ...
        echo "  custom    - 自定义路�?(需设置 CUSTOM_ROUTES, CUSTOM_SCENARIOS)"
        echo ""
        echo "示例:"
        echo "  $0 town05         # Town05 评估，使�?GPU 0"
        echo "  $0 42routes       # 42 Routes 评估，使�?GPU 0"
        echo "  GPU_ID=1 $0 town05  # Town05 评估，使�?GPU 1"
        exit 1
        ;;
esac
...
# 检�?CARLA 服务器是否运�?echo "检�?CARLA 服务器连�?.."
timeout 5 bash -c "echo > /dev/tcp/localhost/2000" 2>/dev/null
if [ $? -eq 0 ]; then
    echo "�?CARLA 服务器已连接"
    echo ""
else
    echo "�?警告: 无法连接�?CARLA 服务�?(端口 2000)"
    echo "  请先在另一个终端运�? ./start_carla_server.sh"
    echo ""
