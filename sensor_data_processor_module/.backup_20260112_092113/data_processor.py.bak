...
class SensorDataProcessor:
    ...
    def _init_swinir(self, swinir_config):
        ...
        try:
            ...
            print(f"  请确保 SwinIR 在路径: {SWINIR_PKG_PATH}")
            self.swinir_processor = None
    ...
    def _init_srgan(self, srgan_config):
        ...
        try:
            ...
            if not model_path or not os.path.exists(model_path):
                print(f"⚠️  SRGAN 模型文件不存在: {model_path}")
                print("  跳过 SRGAN 初始化")
                return
            ...
            print(f"  请确保 SRGAN 在路径: {SRGAN_PKG_PATH}")
            self.srgan_processor = None
        except Exception as e:
            print(f"⚠️  SRGAN 初始化失败: {e}")
            self.srgan_processor = None
